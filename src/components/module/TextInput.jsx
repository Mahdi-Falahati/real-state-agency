    ...
    <div class="w-full max-w-sm min-w-[200px] sm:min-w-[350px] my-3">
      <div class="relative">
        <input
          ...
          value={profileData[name].value}
          onChange={ChangeValueHandler}
          class="peer w-full bg-transparent placeholder:text-black text-black text-sm border border-slate-200 rounded-md px-3 py-2 transition duration-300 ease focus:outline-none focus:border-black hover:border-black shadow-sm focus:shadow"
        />
        <label class="absolute font-semibold cursor-text bg-white px-1 right-2.5 top-2.5 text-black text-sm transition-all transform origin-right peer-focus:-top-2 peer-focus:right-2.5 peer-focus:text-xs peer-focus:text-black peer-focus:scale-90">
          {title}
        </label>
      </div>
    </div>
